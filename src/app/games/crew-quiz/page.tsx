...
export default function CrewQuizPage() {
  ...
  const generateQuestions = (): Question[] => {
    ...
    // Crear preguntas de tripulaciones con al menos 3 miembros
    crewGroups.forEach((members, crew) => {
      if (members.length >= 3) {
        questions.push({
          id: `crew-${crew}`,
          question: `Miembros de ${crew}`,
          answers: members,
    ...
    // Crear preguntas de origen con al menos 4 miembros
    originGroups.forEach((members, origin) => {
      if (members.length >= 4) {
        questions.push({
          id: `origin-${origin}`,
          question: `Personajes de ${origin}`,
          answers: members,
          category: 'Origen'
        })
      }
  ...
  const startGame = () => {
    ...
    const initialTime = Math.max(60, randomQuestion.answers.length * 5) // Mínimo 60 segundos, 5 seg por respuesta
  ...
  const submitAnswer = (selectedCharacter?: AnimeCharacter) => {
    ...
    if (selectedCharacter) {
      ...
      // Buscar el personaje por nombre exacto
    ...
    // Verificar si el personaje pertenece a la respuesta correcta
    ...
    if (isCorrect && !alreadyAnswered) {
      ...
      // Verificar si se completó el 100%
      ...
      // Respuesta incorrecta o duplicada: restar 3 segundos
      setGameState(prev => ({
        ...prev,
        userAnswers: [...prev.userAnswers, characterToSubmit.name],
        timeLeft: Math.max(0, prev.timeLeft - 3)
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-6 text-sm">
              ...
              >
                ...
                {gameState.gameStatus === 'waiting' ? 'Comenzar' : 'Nuevo Juego'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {gameState.gameStatus === 'waiting' && (
            <div className="text-center space-y-6">
              <div className="p-8 bg-[#06394f]/60 border border-amber-700/40 rounded-lg shadow shadow-black/40">
                ...
                <h2 className="text-3xl font-bold text-amber-200 mb-4">¡Adivina la Crew!</h2>
                <p className="text-amber-200/80 mb-6 max-w-2xl mx-auto">
                  Se te dará una pregunta sobre tripulaciones, orígenes o grupos especiales de One Piece. 
                  Escribe todos los personajes que recuerdes. ¡Ganarás 5 segundos extra por cada respuesta correcta!
                </p>
                <button 
                  onClick={startGame}
                ...
                <div className="mt-6 text-sm text-amber-200/60">
                  Total de preguntas disponibles: {questions.length}
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
                        ...
                        {filteredSuggestions.map((char) => (
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <div className="text-amber-100 font-medium text-sm">{char.name}</div>
              ...
              {gameState.userAnswers.length > 0 && (
                <div className="p-6 bg-[#06394f]/40 border border-amber-700/40 rounded-lg shadow shadow-black/40">
                  <h3 className="text-lg font-semibold text-amber-200 mb-4">Tus respuestas:</h3>
          ...
          {gameState.gameStatus === 'finished' && gameState.currentQuestion && (
            <div className="space-y-6">
              <div className="p-8 bg-[#06394f]/60 border border-emerald-600/40 rounded-lg text-center shadow shadow-black/40">
                <h2 className="text-3xl font-bold text-emerald-300 mb-4">¡Tiempo terminado!</h2>
                <div className="text-6xl font-bold text-amber-300 mb-2">{gameState.score}%</div>
                <p className="text-amber-200/80 mb-6">
                  Encontraste {gameState.correctAnswers.length} de {gameState.currentQuestion.answers.length} respuestas
                </p>
                
                {/* Mostrar todas las respuestas correctas */}
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-amber-200 mb-4">Respuestas correctas:</h3>
          ...
          <div className="p-6 bg-[#06394f]/50 border border-amber-700/40 rounded-lg text-sm space-y-3 backdrop-blur-sm shadow shadow-black/30">
            ...
            <ul className="list-disc list-inside space-y-1 text-amber-200/70">
              <li>Se te dará una pregunta sobre una tripulación, origen o grupo especial.</li>
              <li>Debes encontrar TODOS los personajes de la base de datos que pertenezcan a esa categoría.</li>
              <li>Escribe el nombre de un personaje y aparecerán sugerencias con fotos para seleccionar.</li>
              <li>Las respuestas correctas aparecen en verde y suman 5 segundos.</li>
              <li>Las respuestas incorrectas aparecen en rojo y restan 3 segundos.</li>
              <li>El objetivo es encontrar el 100% de respuestas antes de que se acabe el tiempo.</li>
            </ul>
          </div>
        </div>
      </div>
